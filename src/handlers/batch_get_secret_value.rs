...
impl Handler for BatchGetSecretValueHandler {
    ...
    async fn handle(db: &Connection, request: Self::Request) -> Result<Self::Response, AwsError> {
        let (errors, secret_values, next_token) = match (request.filters, request.secret_id_list) {
            ...
            (Some(filters), None) => {
                ...
                let (secrets, count) = db
                    .call(move |db| {
                        Ok::<_, AwsError>((
                            ...
                            get_secrets_count_by_filter(db, &filters, false).inspect_err(
                                |error| tracing::error!(?error, "failed to get secrets count"),
                            )?,
                        ))
                    })
                    .await?;
                ...
                (errors, secret_values, next_token)
            }
            ...
            (None, Some(secret_id_list)) => {
                 db.call(move |db| {
                    ...
                    Ok::<_, AwsError>((errors, secret_values, None))
                }).await?
            }
